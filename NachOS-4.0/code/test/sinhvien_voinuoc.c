...
int main(){
	...
	SpaceId fileid_input, fileid_output, fileid_sinhvien, fileid_result; // some file id needed
	...
	file_success = CreateFile("output.txt");
	if(file_success == -1)
		return 1;
	
	// open file input.txt	for read only
	fileid_input = Open("input.txt", 1);
	if(fileid_input == -1)
		return 1;
	
	// open file output.txt de read and write
	fileid_output = Open("output.txt", 0);
	if(fileid_output == -1)
	{
		Close(fileid_input);
		return 1;
	...
	while(SLTD--)
	{
		...
		file_success = CreateFile("sinhvien.txt");
		if(file_success == -1)
		{
			Close(fileid_input);
			Close(fileid_output);
		...
		fileid_sinhvien = Open("sinhvien.txt", 0);
		...
		fileid_result = Open("result.txt", 1);
		if(fileid_result == -1)
		{
			Close(fileid_input);
			Close(fileid_output);
